 ...
 */
public class CampaignGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all campaigns").get("/api/campaigns").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new campaign")
                        .post("/api/campaigns")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created campaign").get("${new_campaign_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created campaign").delete("${new_campaign_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Campaign entity").exec(scn);
