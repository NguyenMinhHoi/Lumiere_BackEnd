...
public class IntegrationWebhookGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all integrationWebhooks")
                ...
                .exec(
                    http("Create new integrationWebhook")
                        ...
                        .body(
                            StringBody(
                                "{" +
                                "\"name\": \"SAMPLE_TEXT\"" +
                                ", \"targetUrl\": \"SAMPLE_TEXT\"" +
                                ", \"secret\": \"SAMPLE_TEXT\"" +
                                ", \"isActive\": null" +
                                ", \"subscribedEvents\": \"SAMPLE_TEXT\"" +
                                "}"
                            )
                        )
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_integrationWebhook_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created integrationWebhook").get("${new_integrationWebhook_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created integrationWebhook").delete("${new_integrationWebhook_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the IntegrationWebhook entity").exec(scn);
