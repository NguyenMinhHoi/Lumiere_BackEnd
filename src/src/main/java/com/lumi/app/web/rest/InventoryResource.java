...
/**
 * REST controller for managing {@link com.lumi.app.domain.Inventory}.
 */
@RestController
...
public class InventoryResource {
    ...
    /**
     * {@code SEARCH  /inventories/_search?query=:query} : search for the inventory corresponding
     * to the query.
     *
    ...
    public ResponseEntity<List<InventoryDTO>> searchInventories(
        ...
        try {
            Page<InventoryDTO> page = inventoryService.search(query, pageable);
            HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(ServletUriComponentsBuilder.fromCurrentRequest(), page);
            return ResponseEntity.ok().headers(headers).body(page.getContent());
