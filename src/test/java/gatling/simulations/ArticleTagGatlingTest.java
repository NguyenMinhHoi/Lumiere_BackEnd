...
public class ArticleTagGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all articleTags").get("/api/article-tags").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new articleTag")
                        .post("/api/article-tags")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"articleId\": 0" + ", \"tagId\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_articleTag_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created articleTag").get("${new_articleTag_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created articleTag").delete("${new_articleTag_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ArticleTag entity").exec(scn);
