...
public class ClothProductMapGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all clothProductMaps").get("/api/cloth-product-maps").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new clothProductMap")
                        .post("/api/cloth-product-maps")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_clothProductMap_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created clothProductMap").get("${new_clothProductMap_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created clothProductMap").delete("${new_clothProductMap_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ClothProductMap entity").exec(scn);
