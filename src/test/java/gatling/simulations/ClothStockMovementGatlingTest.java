...
public class ClothStockMovementGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(
                http("Get all clothStockMovements")
                    .get("/api/cloth-stock-movements")
                    .headers(headersHttpAuthenticated)
                    .check(status().is(200))
            )
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new clothStockMovement")
                        .post("/api/cloth-stock-movements")
                        .headers(headersHttpAuthenticated)
                        .body(
                            StringBody(
                                "{" +
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_clothStockMovement_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(
                    exec(
                        http("Get created clothStockMovement").get("${new_clothStockMovement_url}").headers(headersHttpAuthenticated)
                    ).pause(10)
                )
                .exec(http("Delete created clothStockMovement").delete("${new_clothStockMovement_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ClothStockMovement entity").exec(scn);
