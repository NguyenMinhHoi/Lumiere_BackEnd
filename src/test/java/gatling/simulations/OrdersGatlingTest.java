 ...
 */
public class OrdersGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .on(
            exec(http("Get all orders").get("/api/orders").headers(headersHttpAuthenticated).check(status().is(200)))
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created orders").get("${new_orders_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created orders").delete("${new_orders_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Orders entity").exec(scn);
