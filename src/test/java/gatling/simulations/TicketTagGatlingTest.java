...
public class TicketTagGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all ticketTags").get("/api/ticket-tags").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new ticketTag")
                        .post("/api/ticket-tags")
                        .headers(headersHttpAuthenticated)
                        .body(StringBody("{" + "\"ticketId\": 0" + ", \"tagId\": 0" + "}"))
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_ticketTag_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created ticketTag").get("${new_ticketTag_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created ticketTag").delete("${new_ticketTag_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the TicketTag entity").exec(scn);
