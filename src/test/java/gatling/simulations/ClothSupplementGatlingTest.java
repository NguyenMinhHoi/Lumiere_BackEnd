...
public class ClothSupplementGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all clothSupplements").get("/api/cloth-supplements").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new clothSupplement")
                        .post("/api/cloth-supplements")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_clothSupplement_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created clothSupplement").get("${new_clothSupplement_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created clothSupplement").delete("${new_clothSupplement_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the ClothSupplement entity").exec(scn);
