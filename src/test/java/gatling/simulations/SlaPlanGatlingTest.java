...
public class SlaPlanGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all slaPlans").get("/api/sla-plans").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new slaPlan")
                        .post("/api/sla-plans")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_slaPlan_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created slaPlan").get("${new_slaPlan_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created slaPlan").delete("${new_slaPlan_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the SlaPlan entity").exec(scn);
