 ...
 */
public class SurveyGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all surveys").get("/api/surveys").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new survey")
                        .post("/api/surveys")
                ...
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created survey").get("${new_survey_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created survey").delete("${new_survey_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the Survey entity").exec(scn);
