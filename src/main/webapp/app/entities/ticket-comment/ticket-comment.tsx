...
import { getEntities, searchEntities } from './ticket-comment.reducer';
...
export const TicketComment = () => {
  ...
  const ticketCommentList = useAppSelector(state => state.ticketComment.entities);
  const loading = useAppSelector(state => state.ticketComment.loading);
  const totalItems = useAppSelector(state => state.ticketComment.totalItems);
  ...
  return (
    <div>
      <h2 id="ticket-comment-heading" data-cy="TicketCommentHeading">
        <Translate contentKey="lumiApp.ticketComment.home.title">Ticket Comments</Translate>
        <div className="d-flex justify-content-end">
          <Button className="me-2" color="info" onClick={handleSyncList} disabled={loading}>
            <FontAwesomeIcon icon="sync" spin={loading} />{' '}
            <Translate contentKey="lumiApp.ticketComment.home.refreshListLabel">Refresh List</Translate>
          </Button>
          <Link to="/ticket-comment/new" className="btn btn-primary jh-create-entity" id="jh-create-entity" data-cy="entityCreateButton">
            <FontAwesomeIcon icon="plus" />
            &nbsp;
            <Translate contentKey="lumiApp.ticketComment.home.createLabel">Create new Ticket Comment</Translate>
          </Link>
        </div>
      </h2>
      <Row>
            ...
            <FormGroup>
              <InputGroup>
                <Input
                  ...
                  placeholder={translate('lumiApp.ticketComment.home.search')}
                />
      ...
      <div className="table-responsive">
        {ticketCommentList && ticketCommentList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
            ...
            <tbody>
              {ticketCommentList.map((ticketComment, i) => (
                <tr key={`entity-${i}`} data-cy="entityTable">
                  <td>
                    <Button tag={Link} to={`/ticket-comment/${ticketComment.id}`} color="link" size="sm">
                      {ticketComment.id}
                    </Button>
                  </td>
                  ...
                  <td className="text-end">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`/ticket-comment/${ticketComment.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      ...
                      <Button
                        onClick={() =>
                          (window.location.href = `/ticket-comment/${ticketComment.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`)
                        }
                        color="danger"
                        size="sm"
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="lumiApp.ticketComment.home.notFound">No Ticket Comments found</Translate>
            </div>
          )
        )}
      </div>
      {totalItems ? (
        <div className={ticketCommentList && ticketCommentList.length > 0 ? '' : 'd-none'}>
