...
/**
 * REST controller for managing {@link com.lumi.app.domain.Campaign}.
 */
@RestController
...
public class CampaignResource {
    ...
    public ResponseEntity<CampaignDTO> createCampaign(@Valid @RequestBody CampaignDTO campaignDTO) throws URISyntaxException {
        ...
        campaignDTO = campaignService.save(campaignDTO);
        return ResponseEntity.created(new URI("/api/campaigns/" + campaignDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, campaignDTO.getId().toString()))
            .body(campaignDTO);
    }
    ...
    public ResponseEntity<CampaignDTO> updateCampaign(
        ...
        campaignDTO = campaignService.update(campaignDTO);
    ...
    /**
     * {@code SEARCH  /campaigns/_search?query=:query} : search for the campaign corresponding
     * to the query.
     *
