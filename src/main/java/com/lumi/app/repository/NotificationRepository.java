...
public interface NotificationRepository extends JpaRepository<Notification, Long>, JpaSpecificationExecutor<Notification> {
    ...
    @Query(
        value = "select notification from Notification notification left join fetch notification.ticket left join fetch notification.customer left join fetch notification.survey",
    ...
    @Query(
        "select notification from Notification notification left join fetch notification.ticket left join fetch notification.customer left join fetch notification.survey"
    )
    List<Notification> findAllWithToOneRelationships();

    @Query(
        "select notification from Notification notification left join fetch notification.ticket left join fetch notification.customer left join fetch notification.survey where notification.id =:id"
    )
